            ...
            <div class="row">
              <div class="col-md-6 mb-3">
                ...
                <select
                  class="form-select"
                  id="specialty"
                  v-model="profileData.specialtyId"
                  required
                >
                  <option
          ...
          <div v-else>
            <div class="row">
              ...
              <div class="col-md-6">
                ...
                <p><strong>Специальность:</strong> {{ profileData.specialtyName }}</p>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async fetchProfile() {
      ...
      try {
        ...
        if (response.status === 'success') {
          this.profileData = response.data.doctor
          this.originalProfileData = { ...response.data.doctor }
        } else {
    ...
    async updateProfile() {
      ...
      try {
        const response = await DoctorProfileService.updateProfile({
          ...
          specialtyId: this.profileData.specialtyId
        })

        if (response.status === 'success') {
          // Обновляем данные профиля с учетом возможных изменений с сервера
          this.profileData = response.data.doctor
          this.originalProfileData = { ...response.data.doctor }
    ...
    async changePassword() {
      ...
      try {
        const response = await DoctorProfileService.changePassword(this.passwordData)
